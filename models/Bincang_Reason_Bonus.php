...
class Bincang_Reason_Bonus
{
    private $conn;
    private $table = "bincang_reason_bonus";
    ...
    {
        $stmt = $this->conn->prepare("SELECT * FROM bincang_reason_bonus WHERE reason_deduction_uuid = ? AND deleted_at IS NULL");
        $stmt->execute([$uuid]);
    ...
    public function create($data)
    {
        $stmt = $this->conn->prepare("
            INSERT INTO bincang_reason_bonus (reason_bonus_uuid, salary_uuid, user_uuid, reason, created_at, updated_at) 
            VALUES (?, ?, ?, ?, ?, ?)
        ");
         $stmt->execute([
            $data['reason_bonus_uuid'],
            $data['salary_uuid'],
            $data['user_uuid'],
            $data['reason'],
            $data['created_at'],
        ...
        if($affectedRows > 0){
            return [
                "status" => "success",
                "code" => 200,
                "data" => $this->getSingleRecordForResponse($data['reason_bonus_uuid'])
            ];
        }
    }

    public function update($reason_bonus_uuid, $data)
{
    // Cek ID
    if (!$reason_bonus_uuid) {
        return errorResponse("400", "reason_bonus_uuid is required");
    }

    // Ambil data lama
    $sqlOld = "SELECT * FROM bincang_reason_bonus WHERE reason_bonus_uuid = :id";
    $stmtOld = $this->conn->prepare($sqlOld);
    $stmtOld->bindValue(':id', $reason_bonus_uuid);
    $stmtOld->execute();
    ...
    $mergedData['reason_bonus_uuid']  = isset($data['reason_bonus_uuid']) ? $data['reason_bonus_uuid'] : $oldData['reason_bonus_uuid'];

    // Eksekusi update
    $sql = "UPDATE bincang_reason_bonus SET 
                reason = :reason,
                user_uuid = :user_uuid,
                reason_bonus_uuid = :reason_bonus_uuid,
                updated_at = :updated_at
            WHERE reason_bonus_uuid = :id 
    ...
    $stmt->execute([
        ...
        ':reason_bonus_uuid' => $mergedData['reason_bonus_uuid'],
        ':updated_at'     => time(),
        ':id'             => $reason_bonus_uuid,
    ]);
    ...
    if ($affectedRows > 0) {
        ...
        $sqlSelect = "SELECT r.*, u.user_username AS reason_by_username 
                      FROM bincang_reason_bonus r
                      JOIN bincang_user u ON r.user_uuid = u.user_uuid
                      WHERE r.reason_bonus_uuid = :id";

        $stmtSelect = $this->conn->prepare($sqlSelect);
        $stmtSelect->bindValue(':id', $reason_bonus_uuid);
        $stmtSelect->execute();
    ...
    public function getSingleRecordForResponse($reason_bonus_uuid){
        $sql = "SELECT c.*, u.user_username AS reason_by_username  
        FROM {$this->table} c
        JOIN bincang_user u ON c.user_uuid = u.user_uuid
        WHERE reason_bonus_uuid = :reason_bonus_uuid";

$stmt = $this->conn->prepare($sql);
$stmt->bindValue(':reason_bonus_uuid', $reason_bonus_uuid, PDO::PARAM_STR);
...
return $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function delete($reason_bonus_uuid, $deleted_by)
{
    // Ambil data sebelum dihapus
    $sqlSelect = "SELECT r.*, u.user_username AS reason_by_username 
                  FROM bincang_reason_bonus r
                  JOIN bincang_user u ON r.user_uuid = u.user_uuid
                  WHERE r.reason_bonus_uuid = :reason_bonus_uuid";

    $stmtSelect = $this->conn->prepare($sqlSelect);
    $stmtSelect->execute([':reason_bonus_uuid' => $reason_bonus_uuid]);
    $item = $stmtSelect->fetch(PDO::FETCH_ASSOC);
    ...
    // Soft delete
    $sql = "UPDATE bincang_reason_bonus 
...
public function recover($reason_bonus_uuid)
{
    ...
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([':reason_bonus_uuid' => $reason_bonus_uuid]);
